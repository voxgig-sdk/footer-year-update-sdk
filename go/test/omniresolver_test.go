...
import (
	...
	omni "github.com/voxgig-sdk/footer-year-update-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FooterYearUpdateError); is {
				out := map[string]any{
					"name":    "FooterYearUpdateError",
...
func omniCtx(arg any, client *sdk.FooterYearUpdateSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FooterYearUpdateSDK); is {
			client = live
