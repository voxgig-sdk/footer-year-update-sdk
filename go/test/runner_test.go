...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("FOOTER_YEAR_UPDATE_TEST_LIVE") == "TRUE" ||
		os.Getenv("FOOTER_YEAR_UPDATE_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("FOOTER_YEAR_UPDATE_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.FooterYearUpdateSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.FooterYearUpdateSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.FooterYearUpdateError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.FooterYearUpdateSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.FooterYearUpdateError{Msg: msg, Code: code}
}
