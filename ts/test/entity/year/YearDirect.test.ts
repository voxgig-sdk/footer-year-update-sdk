...
describe('YearDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new FooterYearUpdateSDK({
  ...
  test('direct-load-year', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-year', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['FOOTER_YEAR_UPDATE_TEST_YEAR_ENTID']
