...
describe('YearEntity', async () => {
  ...
  // `test.live.delayMs`; only sleeps when FOOTERYEARUPDATE_TEST_LIVE=TRUE.
  afterEach(liveDelay('FOOTERYEARUPDATE_TEST_LIVE'))
  ...
  test('basic', async (t) => {
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set FOOTER_YEAR_UPDATE_TEST_YEAR_ENTID JSON to run live')
      return
    }
    ...
    const year_ref01_data_dt0 = await year_ref01_ent.load(year_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['FOOTER_YEAR_UPDATE_TEST_YEAR_ENTID']
