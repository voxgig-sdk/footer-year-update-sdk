...
//     (\`new FooterYearUpdateSDK(...)\` / \`FooterYearUpdateSDK.test(...)\` become
//     \`FooterYearUpdateSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'FooterYearUpdateSDK'
...
const TEST_SEED = {"entity":{"year":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new FooterYearUpdateSDK(...)\` and any
// \`FooterYearUpdateSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+FooterYearUpdateSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/FooterYearUpdateSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
