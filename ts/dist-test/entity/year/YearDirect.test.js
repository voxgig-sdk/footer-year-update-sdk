...
(0, node_test_1.describe)('YearDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.FooterYearUpdateSDK({
    ...
    (0, node_test_1.test)('direct-load-year', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-year', setup.live))
            return;
...
function liveScenariosActive() { return false && process.env.FOOTER_YEAR_UPDATE_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.FooterYearUpdateSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['FOOTER_YEAR_UPDATE_TEST_YEAR_ENTID'];
    ...
    const client = new __1.FooterYearUpdateSDK({
        base: 'http://localhost:8080',
