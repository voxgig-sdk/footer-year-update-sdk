# footer-year-update-mcp

[MCP](https://modelcontextprotocol.io) server exposing the FooterYearUpdate SDK as
two agent tools — `footer-year-update_list` and `footer-year-update_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/footer-year-update-mcp)
make build

# 2. Provide credentials via the environment
export FOOTER_YEAR_UPDATE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user footer-year-update \
  -- /absolute/path/to/footer-year-update-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./footer-year-update-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// footer-year-update_list: first page of records
{ "entity": "year" }
{ "entity": "year", "query": { } }

// footer-year-update_load: one record by id
{ "entity": "year", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/footer-year-update-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user footer-year-update \
     -- "$PWD"/dist/*/footer-year-update-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `footer-year-update_list` and `footer-year-update_load` tools now appear
   in new sessions. Ask the agent to *"list year using footer-year-update"*
   and it calls `footer-year-update_list` with `{"entity":"year"}`.
...
```sh
export FOOTER_YEAR_UPDATE_APIKEY=sk_live_xxx            # API key
export FOOTER_YEAR_UPDATE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `footer-year-update_list` tool
...
### Call the `footer-year-update_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `footer-year-update_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `footer-year-update_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FOOTER_YEAR_UPDATE_APIKEY` | API key sent with every request. |
| `FOOTER_YEAR_UPDATE_BASE` | Optional override of the API base URL. |
...
```sh
./footer-year-update-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"footer-year-update_load","arguments":{"entity":"year","query":{"id":1}}}}'
```
