    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set FOOTER_YEAR_UPDATE_TEST_YEAR_ENTID JSON to run live")
        client = setup["client"]
...
def _year_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("FOOTER_YEAR_UPDATE_TEST_YEAR_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("FOOTER_YEAR_UPDATE_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
            },
            extra or {},
        ])
        client = FooterYearUpdateSDK(helpers.to_map(merged_opts))

    _live = env.get("FOOTER_YEAR_UPDATE_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("FOOTER_YEAR_UPDATE_TEST_EXPLAIN") == "TRUE",
        "live": _live,
