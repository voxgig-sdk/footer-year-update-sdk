# FooterYearUpdate SDK test runner
...
class FooterYearUpdateTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FooterYearUpdateTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FooterYearUpdateTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FooterYearUpdateTestRunner.getenv("FOOTER_YEAR_UPDATE_TEST_LIVE")
        override = FooterYearUpdateTestRunner.getenv("FOOTER_YEAR_UPDATE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FooterYearUpdateTestRunner.getenv(key)
        ...
        explain = FooterYearUpdateTestRunner.getenv("FOOTER_YEAR_UPDATE_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if FooterYearUpdateTestRunner._test_control is not None:
            return FooterYearUpdateTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FooterYearUpdateTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FooterYearUpdateTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FooterYearUpdateTestRunner.load_test_control()
...
def load_env_local():
    FooterYearUpdateTestRunner.load_env_local()


def env_override(m):
    return FooterYearUpdateTestRunner.env_override(m)


def entity_data(v):
    return FooterYearUpdateTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return FooterYearUpdateTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FooterYearUpdateTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FooterYearUpdateTestRunner.load_test_control()


def live_client_options():
    return FooterYearUpdateTestRunner.live_client_options()


def live_delay_ms():
    return FooterYearUpdateTestRunner.live_delay_ms()
