...
// FooterYearUpdate SDK feature test
//
...
class FtCtrl extends FooterYearUpdateControl
{
...
class FtClient
{
    ...
    private FooterYearUpdateUtility $utility;

    public function __construct(FooterYearUpdateUtility $utility, array $options)
    {
    ...
    public function get_utility(): FooterYearUpdateUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FooterYearUpdateContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FooterYearUpdateContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FooterYearUpdateContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FooterYearUpdateConfig::make_feature($name);
    ...
    private static function buildUrl(FooterYearUpdateSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FooterYearUpdateError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FooterYearUpdateResponse($fetched);
        }
    ...
    private function fail(FooterYearUpdateContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FooterYearUpdateContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
