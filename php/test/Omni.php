...
//    only - a typed FooterYearUpdateContext there would make every ctx
...
final class FooterYearUpdateOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new FooterYearUpdateContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new FooterYearUpdateSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new FooterYearUpdateResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new FooterYearUpdateError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new FooterYearUpdateResponse($clean['response']);
    ...
    public static function err_from_map($m): ?FooterYearUpdateError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new FooterYearUpdateError(is_string($code) ? $code : '', $msg);
    }
