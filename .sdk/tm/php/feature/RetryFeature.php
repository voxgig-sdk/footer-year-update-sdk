...
// FooterYearUpdate SDK retry feature
...
class FooterYearUpdateRetryFeature extends FooterYearUpdateBaseFeature
{
    ...
    public function init(FooterYearUpdateContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (FooterYearUpdateContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(FooterYearUpdateContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(FooterYearUpdateContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
