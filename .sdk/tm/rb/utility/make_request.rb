...
module FooterYearUpdateUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = FooterYearUpdateResponse.new({})
    ...
    if fetch_err
      ...
      response = FooterYearUpdateResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FooterYearUpdateResponse.new(fetched)
    else
