// Command FooterYearUpdate-cli is a CLI + REPL for the FooterYearUpdate SDK,
...
const prompt = "footer-year-update"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "year"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      FOOTER_YEAR_UPDATE_APIKEY (api key), FOOTER_YEAR_UPDATE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: FOOTER_YEAR_UPDATE_APIKEY carries the API key and
	// FOOTER_YEAR_UPDATE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("FOOTER_YEAR_UPDATE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("FOOTER_YEAR_UPDATE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewFooterYearUpdateSDK(opts)
