# footer-year-update-cli

boru-driven command-line client **and** interactive REPL for the FooterYearUpdate
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/footer-year-update-cli)
make build

# 2. See usage (words, entities, env vars)
./footer-year-update-cli --help

# 3. Provide credentials once, via the environment
export FOOTER_YEAR_UPDATE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./footer-year-update-cli load 1 year            # {id:1} shorthand
./footer-year-update-cli load '{id:1}' year       # explicit match map

# 5. Override the API base URL for a single call
FOOTER_YEAR_UPDATE_BASE=https://api.example.com ./footer-year-update-cli load 1 year

# 6. No arguments -> interactive REPL
./footer-year-update-cli
footer-year-update> load 1 year
footer-year-update> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/footer-year-update-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/footer-year-update-cli load 1 year
   ```
...
```sh
./footer-year-update-cli load 1 year          # scalar shorthand for {id:1}
./footer-year-update-cli load '{id:1}' year     # explicit match map
```
...
./footer-year-update-cli load 1 year
```
...
Run with no arguments to open a REPL (prompt `footer-year-update>`). Each line is
evaluated as its own boru expression:

```text
$ ./footer-year-update-cli
footer-year-update> load 1 year
footer-year-update> /help
footer-year-update> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `year`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/footer-year-update-cli`. |
...
(`./footer-year-update-cli <expr>`) and interactively (the REPL), and expressions compose the
