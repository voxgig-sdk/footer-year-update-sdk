...
require_relative 'FooterYearUpdate_types'
...
class FooterYearUpdateSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = FooterYearUpdateHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = FooterYearUpdateHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, FooterYearUpdateFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    FooterYearUpdateUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = FooterYearUpdateHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = FooterYearUpdateHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = FooterYearUpdateHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => FooterYearUpdateError.new(
        "#{op}_allow",
        "FooterYearUpdateSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue FooterYearUpdateError => err
    ...
    if fetched.is_a?(Hash)
      status = FooterYearUpdateHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = FooterYearUpdateError.new(
        "graphql_error", "FooterYearUpdateSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = FooterYearUpdateSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
