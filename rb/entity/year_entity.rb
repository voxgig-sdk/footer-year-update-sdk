...
class YearEntity
  ...
  def data_set(args)
    if args
      @_data = FooterYearUpdateHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = FooterYearUpdateHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # Load a single Year.
  #
  # @param reqmatch [YearLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Year.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Year, Hash] the loaded Year; raises FooterYearUpdateError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = FooterYearUpdateHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
